...
/* ════════════════════════════════════════════════
   8. LOBBY WALK — both characters walk to escalator
   Note: Full implementation is in nav.js
   This is kept for backward compat but nav.js handles
   the two-character walk directly now
════════════════════════════════════════════════ */
function animateLobbyWalk(onComplete) {
  // nav.js handles two-character walk — just call complete
  // This function is bypassed when zone elements exist
  const lc = document.getElementById('lobby-char');
  if (lc) {
    gsap.set(lc, { left:'50%', xPercent:-50, opacity:1 });
    lc.classList.add('lc-walking');
    gsap.to(lc, { left:'105%', xPercent:0, duration:1.6, ease:'power1.inOut',
      onComplete: () => { lc.classList.remove('lc-walking'); gsap.set(lc,{opacity:0}); if(onComplete) onComplete(); }
    });
  } else {
